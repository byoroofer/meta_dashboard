...
export const marketplaceDealsRepository = {
  async listSavedSearches() {
    ...
    if (!client) return getDemoStore().savedSearches;
  ...
  async recordSourceError(scanId: string, source: string, errorMessage: string) {
    ...
    const result = await client
      .from("marketplace_source_errors")
      .insert({ scan_id: scanId, source, error_message: errorMessage });
  ...
  async listScanHistory() {
    const client = getSupabaseAdminClient();
    if (!client) return getDemoStore().scans;
      ...
      return store.scanResults
        ...
        .map((result) => {
          ...
          return {
            ...
            analysis: store.analyses[result.id],
